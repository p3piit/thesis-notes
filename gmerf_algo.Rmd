...
In the generalized mixed-effects random forest (GMERF) model, the linear predictor is composed of a fixed component $f(x_{ij})$ and a random component $z_{ij}^{\top} b_i$, such that
\[
\eta_{ij} = f(x_{ij}) + z_{ij}^{\top} b_i,
\qquad 
b_i \sim \mathcal{N}(0, D),
\]
where $f(x_{ij})$ is an unknown function representing the fixed effects, estimated nonparametrically by means of a random forest, $b_i$ is a vector of cluster-specific random effects with covariance matrix $D$ and $z_{ij}$ is the vector for the random effects.

### Algorithm

The GMERF algorithm is the ML-based PQL-EM-algorithm in which we replace the linear structure used to estimate the fixed part of the model by a standard regression forest structure. The algorithm is as follows:
...
\]
with $g(\cdot)$ the logit link, $g'(\mu_{ij}) = [\mu_{ij}(1-\mu_{ij})]^{-1}$, and $w_{ij}^{(0)} = \big(v_{ij}\,g'(\hat\mu_{ij}^{(0)})^2\big)^{-1}$. For Bernoulli outcomes, $v_{ij}=\mu_{ij}(1-\mu_{ij})$, which yields the familiar working weights $w_{ij}^{(0)}=\hat\mu_{ij}^{(0)}\!\big(1-\hat\mu_{ij}^{(0)}\big)$. Using $(\tilde y_i^{(0)}, W_i^{(0)})$, fit a \emph{weighted} linear mixed-effects pseudo-model to obtain initial estimates $\hat\sigma^{2(0)}$ and $\hat D^{(0)}$. These values initialize the subsequent weighted MERT iterations that update the tree-based fixed part and the random effects under the PQL framework.\footnote{See GMERF algorithm initialization and binary special case for the definitions of $\tilde y$ and $w_{ij}$.}

**Outer loop**

After obtaining the initial estimates of the working responses $\tilde y_i^{(0)}$, the weights $W_i^{(0)}$, and the variance components $\hat\sigma^{2(0)}$ and $\hat D^{(0)}$, the outer loop is initialized by setting $M = 0$. This loop governs the successive updates of the working quantities $(\eta_i, \mu_i, \tilde y_i, W_i)$ within the PQL framework. At each outer iteration, the algorithm calls the inner weighted MERF procedure to update the fixed and random components, after which the working responses and weights are recomputed. The outer loop proceeds until convergence of the linear predictors $\eta_i$.
...
\]
A weighted random forest is then fitted to the data $(\tilde y_i^{\ast(m)}, X_i, W_i^{(M)})$ using a standard tree algorithm, thereby yielding an updated estimate of the fixed component, denoted $\hat f^{(m)}(X_i)$. 
