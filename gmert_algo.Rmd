...
In the generalized mixed-effects regression tree (GMERT) model, the linear predictor is composed of a fixed component $f(x_{ij})$ and a random component $z_{ij}^{\top} b_j$, such that
\[
\eta_{ij} = f(x_{ij}) + z_{ij}^{\top} b_j,
\qquad 
b_j \sim \mathcal{N}(0, D),
\]
where $f(x_{ij})$ is an unknown function representing the fixed effects, estimated nonparametrically by means of a regression tree, $b_j$ is a vector of cluster-specific random effects with covariance matrix $D$ and $z_{ij}$ is the vector for the random effects.
...
After obtaining the initial estimates of the working responses $\tilde y_{ij}^{(0)}$, the weights $W_j^{(0)}$, and the variance components $\hat\sigma^{2(0)}$ and $\hat D^{(0)}$, the outer loop is initialized by setting $M = 0$. This loop governs the successive updates of the working quantities $(\eta_{ij}, \mu_{ij}, \tilde y_{ij}, W_j)$ within the PQL framework. At each outer iteration, the algorithm calls the inner weighted MERT procedure to update the fixed and random components, after which the working responses and weights are recomputed. The outer loop proceeds until convergence of the linear predictors $\eta_{ij}$.
...
\[
\tilde y_{ij}^{\ast(m)} = \tilde y_{ij}^{(M)} - Z_j\,\hat b_j^{(m-1)}, \qquad j = 1, \ldots, n.
\]
A weighted regression tree is then fitted to the data $(\tilde y_j^{\ast(m)}, X_j, W_j^{(M)})$ using a standard tree algorithm, thereby yielding an updated estimate of the fixed component, denoted $\hat f^{(m)}(X_j)$. 
...
Here we put the algorithm in a single function with many parameter to be able to control the convergence and to be able to tune the tree from step 1. 
